...
class Model:
    ...
    def _create_dual_objective(self, primal, dual):
        coefficients = [constraint.bound for constraint in primal.constraints]
        dual.minimize(ex.Expression.from_vectors(dual.variables, coefficients))

    def _create_dual_constraints(self, primal, dual):
        matrix = np.array([constraint.expression.factors(primal) for constraint in primal.constraints])
        new_matrix = matrix.transpose()
        for row, bound in zip(new_matrix, primal.objective.expression.factors(primal)):
            expression = ex.Expression.from_vectors(dual.variables, row)
            dual.add_constraint(expression >= bound)
